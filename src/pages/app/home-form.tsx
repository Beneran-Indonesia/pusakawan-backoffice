...
import { PostSchema, Post } from "@/types/app/app-home-type";
...
export default function AppHomeForm() {
  ...
  const editData = query?.data?.data as Post | undefined;

  const authorName = editData?.author.name ?? name ?? "User";
  const authorAvatar = editData?.author.avatar ?? avatar;
  const createdAt = editData?.created_at ?? "";
    ...
    >
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6 space-y-6">
        ...
        <div>
          ...
          />
          <ErrorLabel errors={errors?.pictures as FieldError} />
        </div>
